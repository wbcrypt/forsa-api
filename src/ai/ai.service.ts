...
export class AiService {
  ...
  async chat(
    ...
    system?: string,
    maxTokens = 1000,
  ): Promise<{ content: string; demo: false }> {
    if (!this.hasApiKey) {
  ...
  // ─── Score generation ──────────────────────────────────────────────────────
  async score(prompt: string): Promise<{ content: string; demo: false }> {
    if (!this.hasApiKey) {
      throw new Error('AI_KEY_NOT_CONFIGURED')
    }
  ...
  // ─── Demo mode — scripted adaptive responses ───────────────────────────────
  async demoChat(
    messages: Message[],
    studentData?: any,
    ...
    // Determine which pillar to explore based on turn
    if (turnCount === 1) {
...
const DEMO_SCRIPTS = {
  fr: {
    ...
    financial2: (paymentBy: string, hasGuarantor: boolean) =>
      `Je comprends. ${hasGuarantor ? "C'est rassurant d'avoir un garant." : 'Avez-vous réfléchi à la possibilité d\'ajouter un garant pour renforcer votre dossier ?'}\n\nUne question complémentaire : **si une mensualité devenait difficile à honorer un mois donné**, avez-vous une solution de secours ? Une épargne, un proche qui pourrait aider temporairement ?`,
