...
export class ReportsService {
  ...
  async getCeoDashboard(tenantId: string) {
    const [portfolio, pipeline, payments, partners] = await Promise.all([
      ...
      this.dataSource.query<any[]>(
        `SELECT COUNT(DISTINCT pc.partner_id) AS active_partners,
  ...
  async getFinanceDashboard(tenantId: string) {
    const [ledger, receivables, disbursements] = await Promise.all([
      ...
      this.dataSource.query<any[]>(
        `SELECT ud.*, u.name AS university_name
         ...
         ORDER BY ud.disbursed_at DESC LIMIT 50`,
        [tenantId],
      ),
    ]);
  ...
  async getSalesDashboard(tenantId: string) {
    const [funnel, bySource, byUniversity, performance] = await Promise.all([
      ...
      this.dataSource.query<any[]>(
        `SELECT rs.display_name AS source, rs.channel,
                COUNT(a.id) AS leads,
         ...
         WHERE a.tenant_id = $1
         GROUP BY rs.id ORDER BY leads DESC`,
        [tenantId],
      ),
      this.dataSource.query<any[]>(
        `SELECT u.name, COUNT(a.id) AS applications,
                COUNT(a.id) FILTER (WHERE a.current_status = 'active_student') AS active
         FROM applications a
         JOIN universities u ON u.id = a.university_id
         WHERE a.tenant_id = $1
         GROUP BY u.id ORDER BY applications DESC LIMIT 20`,
        [tenantId],
      ),
      this.dataSource.query<any[]>(
        `SELECT u.full_name,
  ...
  async getCollectionsDashboard(tenantId: string) {
    const [overview, aging, topOverdue] = await Promise.all([
      ...
      this.dataSource.query<any[]>(
        `SELECT
         ...
         GROUP BY 1 ORDER BY MIN(CURRENT_DATE - i.grace_due_date)`,
        [tenantId],
      ),
      this.dataSource.query<any[]>(
        `SELECT s.first_name, s.last_name, s.phone_primary,
