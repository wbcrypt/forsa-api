...
function generateAiReport(student: typeof STUDENTS[0], lang: 'fr' | 'ar' | 'en' = 'fr') {
  ...
  return {
    // T-211/D-003 — field names match household-stability.util.ts's
    // HouseholdStabilityDimensionScores; ai_score_overall is computed
    // server-side from these and stored on the application row itself,
    // never read back out of this JSON (see applications.service.ts).
    scores: {
