...
async function seedDemo() {
  ...
  for (const uni of UNIVERSITIES) {
    ...
    if (existing.length === 0) {
      await ds.query(
        `INSERT INTO universities (id, tenant_id, name, code, city, country_code, status, created_at, updated_at)
      ...
      for (const program of uni.programs) {
        await ds.query(
          `INSERT INTO programs (tenant_id, university_id, name, status, created_at, updated_at)
           VALUES ($1, $2, $3, 'active', NOW(), NOW())
           ON CONFLICT DO NOTHING`,
          [TENANT_ID, uni.id, program]
        )
      }
      seededCount++
    } else {
  ...
  // Seed partner
  const partnerExists = await ds.query(
    `SELECT id FROM partners WHERE email = 'partner@forsa.tn' AND tenant_id = $1 LIMIT 1`,
    [TENANT_ID]
  )
  if (partnerExists.length === 0) {
    await ds.query(
      `INSERT INTO partners (tenant_id, name, type, status, email, country_code, is_founding_partner, created_at, updated_at)
  ...
  for (const s of STUDENTS) {
    ...
    if (existingStudent.length > 0) {
      ...
      const [newStudent] = await ds.query(
        `INSERT INTO students (tenant_id, user_id, first_name, last_name, email, phone, city, nationality, academic_level, status, created_at, updated_at)
      ...
      // Create payment schedule for active students
      if (s.totalPayments > 0 && newApp) {
        const [schedule] = await ds.query(
          `INSERT INTO payment_schedules (tenant_id, student_id, application_id, total_amount, currency, installment_count, start_date, payment_model, status, created_at, updated_at)
           VALUES ($1, $2, $3, $4, 'TND', $5, NOW(), 'concurrent', 'active', NOW(), NOW())
           RETURNING id`,
          [TENANT_ID, studentId, newApp.id, s.tuition, s.totalPayments]
        )
        ...
        if (schedule) {
          for (let i = 1; i <= s.totalPayments; i++) {
            ...
            const [inst] = await ds.query(
              `INSERT INTO installments (tenant_id, schedule_id, sequence_number, due_date, amount, currency, status, created_at, updated_at)
            ...
            if (isPaid && inst) {
              await ds.query(
                `INSERT INTO payments (tenant_id, installment_id, student_id, amount, currency, paid_at, payment_method, status, recorded_by, created_at, updated_at)
                 VALUES ($1, $2, $3, $4, 'TND', NOW() - INTERVAL '${(s.paymentsPaid - i + 1) * 30} days', 'bank_transfer', 'paid', $5, NOW(), NOW())`,
                [TENANT_ID, inst.id, studentId, (s.tuition / s.totalPayments).toFixed(2), adminId]
              )
            }
          }
        }
