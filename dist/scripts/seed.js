...
async function main() {
    ...
    for (const [code, module, action, desc, isHighImpact] of PERMISSIONS) {
        await ds.query(`INSERT INTO permissions (code, module, action, description, is_high_impact)
       VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (code) DO NOTHING`, [code, module, action, desc, isHighImpact]);
    }
    console.log(`✓ ${PERMISSIONS.length} permissions`);
    for (const [code, name, cat, desc, req] of DOCUMENT_TYPES) {
        await ds.query(`INSERT INTO document_types (code, display_name, category, description, is_required, is_active)
       VALUES ($1,$2,$3,$4,$5,true)
       ON CONFLICT (code) DO NOTHING`, [code, name, cat, desc, req]);
    }
    console.log(`✓ ${DOCUMENT_TYPES.length} document types`);
    for (const [code, name, channel] of REFERRAL_SOURCES) {
        await ds.query(`INSERT INTO referral_sources (code, display_name, channel, is_active)
       VALUES ($1,$2,$3,true)
       ON CONFLICT (code) DO NOTHING`, [code, name, channel]);
    }
    ...
    for (const [code, channel, name, subject, body, active] of NOTIFICATION_TEMPLATES) {
        await ds.query(`INSERT INTO notification_templates (code, channel, name, subject_template, body_template, is_active)
       VALUES ($1,$2,$3,$4,$5,$6)
