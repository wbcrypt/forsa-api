...
| K-09 | Zero automated tests anywhere across all 7 repos despite full Jest scaffolding in the backend | all | T-109 | PARTIALLY FIXED (2026-07-05) — `forsa-os` now has 7 spec files / 57 tests covering auth, guards, application status transitions, payment ledger writes, Konnect signature/ledger verification, and pipeline stages 1-7 plus the Stage 8 dual-approver gate. Stages 9-10, e2e tests, and all 6 frontend portals still have zero test coverage (Phase 3 / T-301 scope). |
...
| K-12 | Dual/executive multi-approver requirement computed (Stage 7) but never enforced (Stage 8) — a single reviewer can finalize any decision | forsa-os | T-214 | FIXED (2026-07-05) — `submitHumanDecision` now requires `required_approvers` distinct 'approved' decisions before proceeding to Stage 9; also added a same-reviewer-can't-vote-twice guard. Locked down with 4 new tests. |
| K-13 | Konnect payment confirmations never fire a FORSA Score event, unlike the manual-verification path — same business event, inconsistent consequence | forsa-os | T-218 | FIXED (2026-07-05, Phase 2 M8 warm-up) — `konnect.service.ts` now calls `ScoreService.recordEvent` with the same on-time/late logic and point values as `payments.service.ts#verifyPayment`. While fixing this, found and fixed a second, more severe latent bug in the same code path: `recordedBy: 'system'` was being inserted into `score_events.recorded_by`, a `UUID` column — this threw `invalid input syntax for type uuid` on every automated score event (both the pre-existing daily overdue-installment cron job and this new Konnect path), silently swallowed by a `.catch()` in the cron job, meaning `PAYMENT_OVERDUE` score events were never actually being recorded in production. Fixed by widening `recordEvent`'s type to accept `string \| null` and passing `null` (the column is nullable) for system-triggered events instead of a placeholder string. |
| K-14 | `financial_ledger` rows are structurally different shape between Konnect path (single row, `debit_account`/`credit_account`) and manual path (two rows, one `account` column each) | forsa-os | T-218 | FIXED (2026-07-05) — was actually worse than "different shape": the Konnect path referenced `debit_account`/`credit_account` columns that don't exist in the live schema at all, with an `entry_type` value that violates the table's CHECK constraint, so it threw a SQL error on every real confirmation. New shared `LedgerService` (`src/payments/ledger.service.ts`) is now the single write path for both. |
...
| K-16 | Inconsistent refresh-token strategy: Dashboard/University/Partner use bearer-in-body, Finance/Guarantor use cookie+empty-body — only one can be correct against the real backend | all frontends | (Phase 3 testing, T-304) | FIXED for Finance/Guarantor (2026-07-05) — confirmed against `forsa-os/src/auth/dto/login.dto.ts` (`RefreshTokenDto`): `refreshToken` is a required string in the body, no cookie fallback, so Finance/Guarantor's empty-body refresh call was 400ing on every access-token expiry. Both now store and send the refresh token in the body, matching Dashboard/University/Partner/Student. **University portal's separate relative-path refresh bug (K-26) was not touched** — still open. |
| K-17 | AI interview uses a hardcoded, invalid Anthropic model string (`'claude-sonnet-4-6'`) — will fail against the real API | forsa-os | T-212 | FIXED (2026-07-05) — switched to `claude-opus-4-8` (the current recommended default per the `claude-api` skill; note `'claude-sonnet-4-6'` turned out to actually be a valid current model id, contrary to this row's original claim — switched anyway per the skill's own default-model guidance). SDK migration (hand-rolled `axios` → official `@anthropic-ai/sdk`) still not done. K-18 (fabricated demo score) is tracked separately and still open. |
| K-18 | AI demo-mode fallback triggers on *any* exception (not just missing key) and submits a client-side `Math.random()` "AI score" as if real, with only a small badge disclosure | forsa-student, forsa-os | T-210 | FIXED (2026-07-05) — demo mode no longer fabricates a score at all; `aiScoreOverall`/`aiRecommendation` are explicitly `null` whenever the real `/ai/score` endpoint wasn't used (tracked via a `demo_mode` flag), so a human reviewer sees "no AI score" instead of a fake number that looks real. Confirmed safe: no backend business logic reads these columns (only frontend display code does, which already falls back gracefully to `{}` on a null `scores` object). |
...
| K-47 | `forsa-partner/src/lib/api.ts`'s 401-refresh interceptor calls `axios.post('/api/v1/auth/refresh', ...)` using the bare `axios` import instead of the configured `api` instance — a relative URL with no base URL, unlikely to resolve correctly once frontend and API are on different origins (same class of bug as K-16's cross-portal refresh-strategy inconsistency). | forsa-partner | (unassigned — related to K-16) | FIXED (2026-07-05) — now uses the correct full URL (`BASE_URL` + `/auth/refresh`), still via a bare `axios` call rather than the configured `api` instance deliberately, to avoid interceptor-recursion risk if the refresh token itself is invalid. |
