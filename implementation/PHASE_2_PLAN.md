...
### M9 — Frontend rebuilds (T-219 covered above; T-220 remainder, T-221 remainder, T-222, T-223, T-224) — ✅ ALL DONE (T-222/T-224 completed 2026-07-06)
**Delivered**:
- **`forsa-student`/`forsa-dashboard`** (T-220/T-221 remainder): done across Milestones 2/4/6/7 as each backend piece landed (Membership, Digital Pass, Household Stability, admin decision flow), plus this milestone's Waiting List page (M6) — the nav scaffolding from Phase 1 now has real functionality behind every section.
...
- **`forsa-finance`** (T-222): `DisbursementsPage.tsx` was a placeholder — `university_disbursements` rows were already being written for real (via the DEE), just no read path; added `GET /execution/disbursements`. "View receipt" button did nothing (`setSelected(p)` with nothing rendering off it) — wired to the same pre-signed-URL pattern every other portal uses. `ReportsPage.tsx`'s "Export CSV" actually dumped raw JSON — rewrote as a real multi-section CSV matching the report's actual (summary + array-sections) shape. **Also found and deleted 2 completely dead files** — `HomePage.tsx` and `pages/payments/PaymentsPage.tsx` were entirely copy-pasted from the *student* portal, unrouted, with broken imports referencing API modules that don't exist in this repo.
- **`forsa-partner`** (T-224) — **applying the standing rule surfaced 3 more live violations of it, not just a re-confirmation**: `applicationsApi.list({partnerId})`, `partnerApi.getDashboard(id)`, and `partnerApi.getCommissions()` all called **staff-permissioned** routes (`application.view`, `partner.view`, `partner.commission.approve` respectively) that no partner-portal account holds — worse, two of the three underlying queries had **no partner-scoping filter at all**, meaning if those permissions were ever broadly granted, any partner would see every other partner's applications/commissions across the tenant. Fixed with new self-scoped `GET /partners/me/applications`, `GET /partners/me/dashboard`, `GET /partners/me/commissions`, mirroring `GET /partners/me`'s already-working T-103 pattern. **Also found `partnerApi.update()` called a `PATCH /partners/:id` route that never existed** — profile editing was unconditionally broken (404), not an identity issue; added `PATCH /partners/me`. 5 new tests.
